        ...
        for idx, curr_client in enumerate(curr_clients):
            ...
            for client_iter in range(max_local_iters):
                ...
                if client_iter == 0 or (client_iter + 1) % log_step == 0 or (client_iter + 1) == max_local_iters:
                    ...
                    # Only apply early stopping after having seen every client once
                    if len(server.optimized_clients) == len(server.clients) and client_iter >= 2000:
                        if stop(scores):
                            logger.info(
...
if __name__ == "__main__":
    ...
    config.dims = dims
    config.dgp = dgp
    config.prior = parse_prior_arg(args.prior)
    config.model_type = model_type
