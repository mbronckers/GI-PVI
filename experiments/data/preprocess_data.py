        ...
        for i, file_name in enumerate(info["files"]):
            ...
            if not info["zipped"]:
                if not os.path.exists(save_location):
                    ...
                    if ("post-download" in info) and (info["post-download"][i] is not None):
                        info["post-download"][i](save_location)

            _data = None

            if ("pre-process" in info) and (info["pre-process"][i] is not None):
                ...
                _data = np.loadtxt(save_location, dtype=str, delimiter=info["delimiter"])
            ...
            print(f"{np.sum(rows_with_missing)}/{rows_with_missing.shape[0]} " f"rows had missing data\n")
...
def process_dataset(data_dir, config):
    ...
    preprocessor = ColumnTransformer(transformers=[("num", scaler, config["numerical_features"]), ("cat", categorical_transformer, config["categorical_features"])])
...
def echocardiogram_preprocess_file(file_location):
    data = np.loadtxt(file_location, dtype=str, delimiter=",")
...
datasets = {
    "abalone": {"base_url": f"{ROOT_URL}/abalone/", "zipped": False, "files": ["abalone.data"], "delimiter": ",", "drop-header": False, "remove-missing": False},
    "adult": {
        "base_url": f"{ROOT_URL}/adult/",
        "zipped": False,
    ...
    "mushroom": {"base_url": f"{ROOT_URL}/mushroom/", "zipped": False, "files": ["agaricus-lepiota.data"], "delimiter": ",", "drop-header": False, "remove-missing": False},
    "credit": {"base_url": f"{ROOT_URL}/credit-screening/", "zipped": False, "files": ["crx.data"], "delimiter": ",", "drop-header": False, "remove-missing": True},
    "bank": {
        "base_url": f"{ROOT_URL}/00222/",
        "zipped": True,
        "zipfile": "bank.zip",
    ...
    "protein": {"base_url": f"{ROOT_URL}/00265/", "zipped": False, "files": ["CASP.csv"], "delimiter": ",", "drop-header": True, "remove-missing": False},
    ...
    "bankruptcy": {
        ...
        "drop-header": False,
        "remove-missing": False,
        "pre-process": [bankruptcy_preprocess_file for _ in range(5)],
    },
    ...
    "drug": {"base_url": f"{ROOT_URL}/00373/", "zipped": False, "files": ["drug_consumption.data"], "delimiter": ",", "drop-header": True, "remove-missing": False},
    "echocardiogram": {
        "base_url": f"{ROOT_URL}/echocardiogram/",
        "zipped": False,
...
abalone_config = {"numerical_features": [1, 2, 3, 4, 5, 6, 7], "categorical_features": [0], "folder": "/abalone", "output_column": 8, "output_generator": abalone_output_generator}
...
protein_config = {"numerical_features": list(range(9)), "categorical_features": [], "folder": "/protein", "output_column": 9, "output_generator": protein_output_generator}
...
power_config = {"numerical_features": list(range(4)), "categorical_features": [], "folder": "/power", "output_column": 4, "output_generator": power_output_generator}
