...
# def scatter_plot(fdir, fname, x1: Tensor, y1: Tensor, desc1: str, xlabel: Optional[str]=None, ylabel: Optional[str]=None, title: Optional[str]=None):
...
def line_plot(fdir, fname, x, y, desc, xlabel=None, ylabel=None, title=None):
    ...
    sns.lineplot(y=get_values(y), x=x, label=f"{desc}", ax=ax, 
                color=colors[0])
