...
def process_partition(
    args, definitions, code_to_group, id_to_group, group_to_id, partition, sample_rate=1.0, shortest_length=4, eps=1e-6, future_time_interval=24.0, fixed_hours=48.0
):
    ...
    for patient in tqdm(patients, desc="Iterating over patients in {}".format(partition)):
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as ts_file:
                ...
                cur_phenotype_labels = [x for (i, x) in enumerate(cur_phenotype_labels) if definitions[id_to_group[i]]["use_in_benchmark"]]
                ...
                if pd.isnull(deathtime):
                    ...
                    lived_time = (datetime.strptime(deathtime, "%Y-%m-%d %H:%M:%S") - datetime.strptime(intime, "%Y-%m-%d %H:%M:%S")).total_seconds() / 3600.0
                ...
                cur_decomp_labels = [(mortality & int(lived_time - t < future_time_interval)) for t in sample_times]
